...
@Component({
     selector: 'que-filter',
    templateUrl: './app/agent-assignment/agent.html',
    styleUrls: ['./app/agent-assignment/agent.css'],
...
export class AgentAssignmentComponent implements OnInit, AfterViewInit  {
    ...
    invalidPercentage: boolean = true;
    pdrAgents: Array<any>;
    assignAgentSaveSuccessful: boolean = false;
          ...
          this.jQuery(document.body).click(function(event) {
            TimerWrapper.setTimeout(() => {
              if(comp.calendarClicked) {
                ...
                comp.datePickerOpen = comp.timePickerPickerOpen = false;
              }
            }, 100);
          });
    }
    ...
    ngOnInit(){
        this.queuingService.getSelectedFilter().subscribe((response)=>{
            ...
            this.filterByDate = new Date(response.SETTING_VAL);
      ...
      this.noSelection = true;
        ...
        for (let key in this.filterDetailsRows) {
            if (this.filterDetailsRows[key].isIncluded) {
                ...
                currentValue = this.filterDetailsRows[key].recordCount * this.filterDetailsRows[key].percentage;
    ...
    saveUpsertPDRAgent(){
        ...
        this.queuingService.upsertPDRAgent(agentSaveDetails).subscribe((res)=>{
            ...
            this.isSaveSuccessful = true;
            this.saveMessage = <any>res;
            this.assignAgentSaveSuccessful=true;
        })
    }
